...
class RotaryEmbedding(nn.Module):
    ...
    def precompute_freqs(self, dim, max_seq_len, theta):
        ...
        Returns:
            cos: 余弦值 [max_seq_len, dim]
            sin: 正弦值 [max_seq_len, dim]
        """
        # 计算逆频率: 1 / (theta^(2i/d))
        # inv_freqs: [dim/2]
    ...
    def forward(self, xq, xk):
        ...
        Args:
            xq: 查询张量 [batch_size, seq_len, num_heads, dim]
            xk: 键张量 [batch_size, seq_len, num_heads, dim]

        Returns:
            xq_rotated: 旋转后的查询 [batch_size, seq_len, num_heads, dim]
            xk_rotated: 旋转后的键 [batch_size, seq_len, num_heads, dim]
        """
        seq_len = xq.size(1)

        # 获取当前位置的 cos 和 sin 值
        # cos, sin: [1, seq_len, 1, dim]
        cos = self.cos[:seq_len].view(1, seq_len, 1, self.dim)
        sin = self.sin[:seq_len].view(1, seq_len, 1, self.dim)
